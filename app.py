...
def detect_live():
    """Live-режим: только детекция и быстрая классификация."""
    ...
    tmp_path = UPLOAD_FOLDER / 'live_frame.jpg'
    with open(tmp_path, 'wb') as f:
        f.write(img_bytes)

    # Используем адаптер с пониженным порогом для live
    result = adapter.predict(tmp_path, conf=0.3)   # <- conf=0.3
